...
class Notification
  ...
  def check(event)
    ...
    if sensor_ids.blank?
      ...
      return check_for_src_ip(event.ip_src) unless ip_src.blank?
      return check_for_dst_ip(event.ip_dst) unless ip_dst.blank?
      ...
      if sensor_ids.include?(event.sid.to_s)
        puts 'sensor!'
        return check_for_src_ip(event.ip_src.to_s) unless ip_src.blank?
        return check_for_dst_ip(event.ip_dst.to_s) unless ip_dst.blank?
